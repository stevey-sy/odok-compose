...
class SupabaseAuthServiceImpl @Inject constructor(
    ...
    override suspend fun signInWithGoogle(idToken: String): AuthResult<AuthUser> {
        return try {
            ...
            // Supabase Google OAuth 로그인 - 3.x 버전 방식
            ...
            Log.d(TAG, "Supabase 로그인 성공: ${user?.id}")

            if (session == null || user == null) {
                return AuthResult.Error(
                    exception = IllegalStateException("세션 또는 사용자 정보를 가져올 수 없습니다"),
                    message = "로그인에 실패했습니다"
                )
            }
            ...
            val authUser = AuthUser(
                ...
                tokenExpiresAt = session.expiresAt?.epochSeconds ?: 0L
            )
    ...
    override suspend fun refreshToken(): AuthResult<AuthUser> {
        return try {
            ...
            // 3.x 버전에서는 refreshCurrentSession 사용
    ...
    override suspend fun updateProfile(
        ...
        return try {
            ...
            // 3.x 버전에서는 updateUser 사용
            supabaseClient.auth.updateUser {
                data = buildJsonObject {
                    displayName?.let { put("full_name", it) }
                    photoUrl?.let { put("avatar_url", it) }
                }
            }
...
//    }
    ...
    private suspend fun updateUserInDatabase(authUser: AuthUser) {
        try {
            val existingUser = userDao.getById(authUser.id)
